...
CanadaPost.prototype.getRates = function(params, callback) {
  ...
  /**
   * mod by gabidi@github.com
   * Accept expected mailing date for package date calculations :)
   * Nov 30th 2016
   */
  if (params.expectedMailingDate) {
    xml.ele('expected-mailing-date', params.expectedMailingDate)
  }
    ...
    parseXML(body, function(err, result) {
      ...
      if (result && result['price-quotes'] && result['price-quotes']['price-quote']) {
        for (var q in result['price-quotes']['price-quote']) {
          ...
          var priceDetails = cq['price-details'][0];
          outQuote.price.base = parseFloat(priceDetails.base[0]);
          outQuote.price.gst = parseFloat(priceDetails.taxes[0].gst[0]['_'] || 0);
          outQuote.price.pst = parseFloat(priceDetails.taxes[0].pst[0]['_'] || 0);
          outQuote.price.hst = parseFloat(priceDetails.taxes[0].hst[0]['_'] || 0);
          outQuote.price.total = parseFloat(priceDetails.due[0]);

          if (priceDetails.adjustments && priceDetails.adjustments[0]) {
            outQuote.adjustments = [];

            for (var i = 0; i < priceDetails.adjustments[0].adjustment.length; i++) {
              var adj = priceDetails.adjustments[0].adjustment[i];
              outQuote.adjustments.push({
                code: adj['adjustment-code'][0],
                name: adj['adjustment-name'][0],
                cost: parseFloat(adj['adjustment-cost'][0]),
              })
            }
          }
